...
class IPSEngine:
        ...
        try:
            ...
            if rule.action == "block":
                ...
                success = await self.app_blocker.add_to_blocklist(
                    src_ip, reason=f"IPS Block: {rule.name}"
                )
                result.update(
                    {
                        "success": success,
    ...
    async def _handle_block(
        self, src_ip: str, rule: IPSRule, context: Dict, db: AsyncSession
    ) -> Dict:
        """Advanced IP blocking with multiple techniques"""
        result = {"action": "block", "success": False, "techniques": []}
    ...
    def _check_threshold(self, rule: IPSRule, src_ip: str) -> bool:
        ...
        if key in self.session_timers and (now - self.session_timers[key]) > timedelta(
            seconds=rule.window
        ):
    ...
    async def process_packet(self, packet: Packet):
        ...
        for rule in self.rules:
            ...
            await self._handle_rule_match(
                rule, packet, src_ip, dst_ip, src_port, dst_port, protocol
            )
    ...
    async def _handle_rule_match(
        ...
        await db.execute(
            ...
            .values(
                last_triggered=datetime.now(), true_positives=IPSRule.true_positives + 1
            )
        )
        await db.commit()

        # Take mitigation action
        mitigation_result = await self._mitigate_threat(
            rule, src_ip, dst_ip, dst_port, protocol
        )
    ...
    async def _mitigate_threat(
        self, rule: IPSRule, src_ip: str, dst_ip: str, dst_port: int, protocol: str
    ) -> Dict:
        ...
        try:
            if rule.action == "block":
                ...
                result["message"] = (
                    f"Blocked {src_ip} -> {dst_ip}:{dst_port}/{protocol}"
                )
                result["success"] = True
                ...
                result["message"] = (
                    f"Throttled {src_ip} -> {dst_ip}:{dst_port}/{protocol}"
                )
                result["success"] = True
