...
class PacketSniffer:
    ...
    def __init__(self, log_file):
        ...
        self._dns_counter = defaultdict(
            lambda: {"total": 0, "nxdomain": 0}
        )  # DNS stats
        self._endpoint_tracker = defaultdict(
            lambda: defaultdict(set)
        )  # Track unique endpoints
        self._protocol_counter = defaultdict(Counter)  # Track protocol distribution
        self.recent_packets = defaultdict(
            lambda: deque(maxlen=100)
        )  # Last 100 packets per IP
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if asyncio.iscoroutine(event):
                    ...
                    asyncio.run_coroutine_threadsafe(event, self.loop)
                self.event_queue.task_done()
            except Empty:
                continue
    
    def _setup_logging(self):
        """Configure packet-level logging with console output"""
        self.packet_logger = logging.getLogger("packet_traffic")
        self.packet_logger.setLevel(logging.INFO)

        # Clear existing handlers
        if self.packet_logger.hasHandlers():
            self.packet_logger.handlers.clear()
        ...
        self.packet_logger.addHandler(console_handler)
        self.packet_logger.addHandler(file_handler)

    def _log_to_jsonl(self, packet_info: dict):
        """Thread-safe JSONL logging"""
        try:
            with self.jsonl_lock, open(self.log_file, 'a', encoding='utf-8') as f:
                json.dump(packet_info, f, default=str)
                f.write('\n')
        except Exception as e:
            logger.error(f"JSONL write failed: {str(e)}")
        ...
        try:
            with self._processing_lock:
                ...
                if http_layer:
                    ...
                    logger.info(
                        f"DNS Query: {dns.qname.decode()} (Type:{dns.qtype}) from {src_ip}"
                    )
                    self._analyze_dns(packet)
                elif packet.haslayer(TCP) and (
                    packet[TCP].dport == 22 or packet[TCP].sport == 22
                ):
                    self._endpoint_tracker[src_ip]["tcp"].add(
                ...
                # UDP handling
                if packet.haslayer(UDP):
                    self._endpoint_tracker[src_ip]["udp"].add(
                        (dst_ip, packet[UDP].dport)
                    )
                    packet_info.update(
            ...
            self._log_to_jsonl(packet_info)
            self.packet_logger.info(
            "[%s] %s:%s -> %s:%s | Proto: %s | Size: %d | Flags: %s | %s%s%s%s",
            packet_info["timestamp"].strftime("%H:%M:%S.%f")[:-3],  # mm:ss:ms
            packet_info["src_ip"],
            str(packet_info.get("src_port", "")),
            packet_info["dst_ip"],
            str(packet_info.get("dst_port", "")),
            self._color_protocol(packet_info["protocol"]),
            packet_info["size"],
            self._format_flags(packet_info.get("flags", "")),
            self._format_dns(packet_info),
            self._format_http(packet_info),
            self._format_threats(packet_info),
            self._format_payload_stats(packet_info)
        )
        except Exception as e:
    ...
    def _color_protocol(self, protocol: str) -> str:
        colors = {
            "HTTP": "\033[92m",   # Green
            "HTTPS": "\033[92m",  # Green
            "DNS": "\033[93m",    # Yellow
        ...
        return f"{colors.get(protocol, '')}{protocol}\033[0m"
        ...
        with self._data_lock:
            ...
            if endpoints:
                for proto, ports in endpoints.items():
                    logger.debug(
                        f"Protocol: {proto}, Ports: {ports}, Port types: {[type(p) for p in ports]}"
                    )
    ...
    def _create_alert(
        ...
        logger.critical("Security alert: ", alert)
        return alert
    ...
    def _start_sniffing(self, interface: str):
        """Internal sniffing method with BPF filter and automatic restart"""
        while not self.stop_event.is_set():  # Added while loop for infinite retries
            try:
                ...
                logger.error(f"Sniffing error: {str(e)}. Restarting...")
                time.sleep(5)  # Prevent tight loop on repeated errors
                # Don't call self.stop() here - let it retry
    ...
    def _format_flags(self, flags: str) -> str:
        return ''.join([f for f in flags if f.isupper()]) if flags else '---'

    def _format_dns(self, packet_info: dict) -> str:
        if packet_info.get("dns_query"):
            return f" | DNS Query: {packet_info['dns_query'][:25]}"
        return ""

    def _format_http(self, packet_info: dict) -> str:
        http_info = []
        if packet_info.get("http_method"):
            http_info.append(f"HTTP {packet_info['http_method']}")
        if packet_info.get("http_path"):
            http_info.append(f"Path: {packet_info['http_path'][:30]}")
        return " | ".join(http_info)

    def _format_threats(self, packet_info: dict) -> str:
        if packet_info.get("threat_score", 0) > 30:
            return f" \033[91mTHREAT[{packet_info['threat_type']}]\033[0m"
        return ""

    def _format_payload_stats(self, packet_info: dict) -> str:
        if packet_info.get("payload"):
            payload = packet_info["payload"]
            return f" | Payload: {len(payload)}b (Entropy: {self._calculate_shannon_entropy(payload):.2f})"
        return ""
