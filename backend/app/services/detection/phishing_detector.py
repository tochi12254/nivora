...
if __name__ == "__main__":
    ...
    # Initialize detector
    phishing_detector = ClassicalPhishingDetector()
    phishing_detector.start()

    # Simulate packet data (in real app this comes from your packet sniffer)
    test_packets = [
        {"http": {"host": "paypal.secure-login.com", "path": "/verify"}},
        {"http": {"host": "google.com", "path": "/search?q=python"}},
        {"http": {"host": "bit.ly", "path": "/2mB4dF9"}},
        {"http": {"host": "192.168.1.1", "path": "/login.php"}},
    ]

    # Process packets
    for packet in test_packets:
        phishing_detector.live_monitor(packet)
        time.sleep(0.1)  # Simulate real-time delay

    # Check results
    while not phishing_detector.result_queue.empty():
        alert = phishing_detector.result_queue.get()
        print(f"\n🚨 PHISHING ALERT 🚨")
    ...
    manual_result = phishing_detector.manual_check("https://apple.security.com/verify")
    print(f"\nManual Check Result: {manual_result}")

    # Cleanup
    phishing_detector.stop()
