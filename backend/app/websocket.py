...
class PacketSniffer:
    ...
    def _analyze_http(self, packet: Packet):
        ...
        try:
            logger.info("Starting http analysis!")
            http_layer = None
            http_data = {}
            
            try:
                # ==================== HTTP Layer Extraction ====================
                if packet.haslayer(HTTPRequest):
                    http_layer = packet[HTTPRequest]
                elif packet.haslayer(HTTPResponse):
                    http_layer = packet[HTTPResponse]
                ...
                if not http_layer:
                    logger.debug("No HTTP layer found in packet")
                    return

            except Exception as e:
                logger.error(f"HTTP layer extraction failed: {str(e)}", exc_info=True)
                self.sio_queue.put(('detection_error', {
                    'error': 'http_layer_extraction_failed',
                    'message': str(e),
            ...
            try:
                ...
                self.sio_queue.put(('detection_error', {
                    'error': 'payload_extraction_failed',
                    'message': str(e),
                    'source_ip': packet[IP].src if packet.haslayer(IP) else None
                }))

            # ==================== Base HTTP Data Collection ====================
            try:
                http_data = {
                    "timestamp": datetime.utcnow().isoformat(),
                    "source_ip": packet[IP].src if packet.haslayer(IP) else None,
                ...
                logger.error(f"Base data collection failed: {str(e)}", exc_info=True)
                self.sio_queue.put(('detection_error', {
                    'error': 'base_data_collection_failed',
                    'message': str(e),
                    'packet_summary': packet.summary()
                }))
                return
            ...
            try:
                ...
                logger.error(f"Network metrics collection failed: {str(e)}", exc_info=True)
                self.sio_queue.put(('detection_error', {
                    'error': 'network_metrics_failed',
                    'message': str(e),
                    'source_ip': http_data.get("source_ip")
                }))
                http_data["network_metrics"] = {}
            ...
            try:
                ...
                logger.error(f"Session context creation failed: {str(e)}", exc_info=True)
                self.sio_queue.put(('detection_error', {
                    'error': 'session_context_failed',
                    'message': str(e),
                    'source_ip': http_data.get("source_ip")
                }))
                http_data["session_context"] = {}
            ...
            try:
                ...
                logger.error(f"Security analysis failed: {str(e)}", exc_info=True)
                self.sio_queue.put(('detection_error', {
            ...
            try:
                ...
                if critical_threats:
                    self.sio_queue.put((
                        "critical_alert",
                        {**critical_threats, "raw_packet_summary": packet.summary(), "mitigation_status": "pending"}
                    ))

                if payload:
                    sig_results = self.signature_engine.scan_packet(payload)
                    if sig_results:
                        self.sio_queue.put_nowait(("signature_match", {**sig_results, "context": http_data}))
            except Exception as e:
                logger.error(f"Threat processing failed: {str(e)}", exc_info=True)
                self.sio_queue.put(('detection_error', {
                    'error': 'threat_processing_failed',
                    'message': str(e),
                    'source_ip': http_data.get("source_ip")
                }))

            try:
                logger.info("Ending http analysis!")
                self.sio_queue.put(("http_activity", http_data))
            except Exception as e:
                logger.critical(f"Failed to send HTTP activity data: {str(e)}", exc_info=True)
                self.sio_queue.put(('system_error', {
            ...
            logger.critical(f"Critical failure in HTTP analysis: {str(global_ex)}", exc_info=True)
            self.sio_queue.put(('system_error', {
                'error': 'http_analysis_critical_failure',
                'message': str(global_ex)
            }))
