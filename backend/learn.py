...
class AdvancedFeatureExtractor:
    ...
    def flow_key(self, packet):
        """
        Build a 5‐tuple key for IPv4 or IPv6, and for TCP, UDP, or ICMP.
        Returns None for packets that aren't IP/TCP/UDP/ICMP.
        """
        # IPv4
        ...
        # Transport: ICMP (IPv4) or ICMPv6
        elif packet.haslayer(ICMP):
            l4 = packet[ICMP]
            proto = 1
            # ICMP has no ports—use type/code as pseudo‐ports to distinguish flows
            src_port = l4.type
            dst_port = l4.code

        else:
            # Other protocols aren’t supported in 5-tuple keying
            return None
    ...
    def extract_features(self, packet: Packet) -> Dict[str, Any]:
        ...
        - Always returns at least {'timestamp': ...}
        - Initializes start_time/last_activity to avoid negative durations/IATs
        - Injects timestamp into packet_info for downstream use
        - Minimizes lock scope and separates KeyError vs. other errors
        """
        # 1) Capture a timestamp up front
        timestamp = getattr(packet, "time", None) or time.time()

        # === PART A: Shared‐state updates that may KeyError ===
        try:
            with self.lock:
                # 1a) Cleanup stale flows
                ...
                # 1c) Inject timestamp for downstream use
                ...
                # 1e) Init flow state if new, with proper timing
                now = timestamp
                if flow_key not in self.flows:
                    fs = FlowStatistics()
                    fs.start_time    = now
                ...
                # 1h) Inter-arrival time (IAT), only from 2nd packet onward
                ...
                # 1i) IP-layer metadata (IPv4/IPv6)
                ip_layer = packet[IP] if packet.haslayer(IP) else packet[IPv6]
                flow_stats.unique_dest_ips.add(ip_layer.dst)
                flow_stats.protocols_seen.add(ip_layer.proto)

                # 1j) TCP specifics (including backward PSH/URG)
                if packet.haslayer(TCP):
                    tcp = packet[TCP]
                    if direction == "backward":
                        if tcp.flags & 0x08:
                    ...
                    flow_stats.tcp_options_count         = len(tcp.options or [])

                # 1k) Host-based sliding window
                dport = packet[TCP].dport if packet.haslayer(TCP) else 0
                self.host_windows[ip_layer.src].append((now, length, ip_layer.dst, dport))

        except KeyError as ke:
            logger.error(f"Feature extraction missing field: {ke}")
            return {"timestamp": timestamp}

        # === PART B: Feature computation & export, catch‐all errors ===
        try:
            # 2a) Compute metrics & update
            ...
            # 2b) Build feature dict
            features = self._compute_flow_features(flow_key, packet_info)
            features.update(self._extract_behavioral_features(packet_info))
            features.update(self._extract_protocol_features(packet, packet_info))

            # 2c) Inject timestamp before any export
            features["timestamp"] = timestamp

            # 2d) Export on FIN/RST, then clear
            if packet.haslayer(TCP) and (packet[TCP].flags & 0x05):
                self._append_features_row("ml_ready_features.csv", features)
                self.clear_flow(flow_key)

            return features

        except Exception as e:
            logger.exception(f"Unexpected error in feature computation: {e}")
            return {"timestamp": timestamp}
    ...
    def _compute_flow_features(
        ...
        duration = max(flow_stats.duration, 1e-6)
        if duration > 0:
            features.update(
                {
                    "Flow_Bytes_s": (
                    ...
                    / duration,
                    "Flow_Packets_s": flow_stats.total_packets / duration,
                    "Flow_IAT_Mean": (
        ...
        bwd_times = flow_stats.backward_timestamps
        if len(bwd_times) > 1:
    ...
    def get_flow_summary(self, flow_key: FlowKey) -> Dict[str, Any]:
        ...
        duration = max(flow_stats.duration, 1e-3)
        summary = {
            "flow_key": {
        ...
        iats = flow_stats.flow_iat
        active_windows = []
        threshold = 1.0  # 1 second of idle = new active period

        if iats:
            start = iats[0]
            for i in range(1, len(iats)):
                if iats[i] - iats[i-1] > threshold:
                    active_windows.append(iats[i-1] - start)
                    start = iats[i]
            active_windows.append(iats[-1] - start)

        summary.update({
            "Active_Mean": statistics.mean(active_windows) if active_windows else 0,
            "Active_Std": statistics.stdev(active_windows) if len(active_windows) > 1 else 0,
            "Active_Max": max(active_windows, default=0),
            "Active_Min": min(active_windows, default=0),
        })
